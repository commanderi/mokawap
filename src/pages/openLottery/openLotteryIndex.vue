    ...
    <div class="content">
      <ul class="list-view-section-body">
        <template v-for="(item, index) in dataList">
          <li class="listItem___9-JCA" @click="changePage('PK10/BJPK10', {id:item.id, title: item.name})" :key="index" v-if="item.id==3||item.id==4">
            ...
            <div class="listItem___3Iww8">
              ...
              <div class="flex___16JOt openCode___2FB7l">
                <div :class="['pk10State___2NHa8', 'pk10State'+num]" v-for="(num, i) in item.number" :key="i"></div>
              </div>
            </div>
          </li>
          <li class="listItem___9-JCA" @click="changePage('SSC/CQSSC', {id: item.id, title: item.name})" :key="index" v-else>
            <img class="listItemImg___LLQ1h" :src="item.cate_img" :alt="item.name">
            <div class="listItem___3Iww8">
              <div class="flex___16JOt listItemChild___3-Nq6">
                <div class="gameNameInChinese___2cQL3">{{item.name}}</div>
              ...
              <!-- 新疆时时彩 -->
              <div class="flex___16JOt openCode___2FB7l">
                <div class="num___1g-CB" v-for="(num, i) in item.number" :key="i">{{num}}</div>
