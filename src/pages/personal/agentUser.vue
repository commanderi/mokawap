    ...
    <div class="content">
      <div>
        ...
        <div class="flex___16JOt accountNavList___3nYLs">
          <div class="navItem___2y29q" v-for="(item, index) in navItem" :key="index" @click="navItemFn(item.id, index)">
            {{item.name}}
          </div>
        </div>
      </div>
        ...
        <div class="listContainer___2HPrh">
          ...
          <div v-for="(item, index) in dataList" :key="index" @click="downListFn(index)">
            ...
            <div class="flex___16JOt listItemOption___1LGdc" v-if="item.active">
              <div class="optionItem___1MhVG" @click.stop="xiaJiUserFn(item)">
                <div class="iconItem___2aUkH icon1___dhQ8v"></div>
                下级
              </div>
              <!--<div tappable="true" class="optionItem___1MhVG">
                <div class="iconItem___2aUkH icon2___3c2yb"></div>
                修改
              </div>-->
              <div class="optionItem___1MhVG" @click.stop="changePage('/accountHistoryList', {title:item.mobile+'报表',junior_id: item.id})">
                <div class="iconItem___2aUkH icon5___3mv10"></div>
                报表
              </div>
            </div>
          </div>
